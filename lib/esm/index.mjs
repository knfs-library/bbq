...
import workerDefault from './configs/worker.mjs';
import queueDefault from './configs/queue.mjs';
/**
 * @type {ConfigType}
 */
import configDefault from './configs/dispatcher.mjs';
...
class Dispatcher {
	...
	getQueueById(queueId) {
		if (!this.#queues.has(queueId)) {
			errorFunc(`Queue ${queueId} not exist`);
		}
	...
	async #updateMetadata() {
		...
		metadata.queues = queueArr.map((queue) => {
			return {
				id: queue.id,
				name: queue.name,
				path: queue.getPath(),
